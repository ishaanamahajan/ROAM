...
function renderGroupResults(){
 const root=document.querySelector("#group-results"),library=profileLibrary(),names=[...document.querySelectorAll(".member-check:checked")].map(x=>x.value),fairness=Number(document.querySelector("#fairness")?.value||35)/100;
 ...
 const group=D.map((_,i)=>{const values=matrices.map(row=>row[i]),mean=values.reduce((a,b)=>a+b,0)/values.length,min=Math.min(...values),m=values.reduce((a,b)=>a+b,0)/values.length,dis=Math.sqrt(values.reduce((s,v)=>s+(v-m)**2,0)/values.length);return{index:i,score:(1-fairness)*mean+fairness*min,dis};});
 ...
 root.innerHTML=`<h2>The shortlist</h2><p class="lede">Balanced for ${names.length} travelers · fairness setting ${Math.round(fairness*100)}%</p><div class="group-list">${group.slice(0,6).map((g,rank)=>{const d=D[g.index];return `<article class="group-card"><img src="${image(d)}" alt="Illustrated ${esc(d.name)} travel postcard"><div><h3>${rank+1}. ${esc(d.name)}, ${esc(d.country)}</h3><p>${esc(d.blurb)}</p><span class="pill">${esc(d.best)} · ${g.dis<.55?"easy consensus":"some tradeoffs"}</span></div><div class="group-score"><strong>${Math.round(g.match)}%</strong><span>GROUP MATCH · DISAGREEMENT ${g.dis.toFixed(2)}</span></div></article>`;}).join("")}</div>`;
}
